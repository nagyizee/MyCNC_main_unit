...
const char cmd_spindle[] =  { 0xA9, 0x14, 0x00, 0xEE };
    ...
    else
    {
        ...
        switch ( fe.opstat.phase )
        {
            ...
            case opstat_spsp_set_evmask2:       // event mask set for jam only
                ...
                if ( internal_command_get_events( FE_EV_SPINDLE_JAM | FE_EV_SPINDLE_OK ) )
                    goto _error_exit;
                fe.opstat.retries = FE_MSG_RETRY;
                fe.opstat.phase++;
                break;
...
static inline void local_poll_coodinates_get( struct SEventStruct *evt )
{
    int res;
 ...
 * *************************************************/
    ...
    int front_end_request_rpm( uint32 *rpm )
    {
        if ( fe.op )
            return -1;
    ...
    int front_end_request_coordinates( void )
    {
        ...
        if ( fe.in_use )
        {
            ...
            fe.opstat.retries = FE_MSG_RETRY;
            fe.status.coord_updated = false;     // indicating invalid rpm
        }
        else
        {
            fe.op = feop_spindle_set_rpm;
