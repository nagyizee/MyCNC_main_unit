...
int mainw::HW_wrp_simu_datafeed()
{
    ...
    if ( comm_rx.resp_ct )
    {
        ...
        if ( commwait_getstatus && (comm_rx.resp[0] == RESP_ACK) )
        {
            ...
            sprintf(  str, "[%c%c%c%c %c%c%c%c][%c%c%c%c] code:%02d | free:%02d execID:%03d QID:%03d",
                     (comm_rx.resp[2] & 0x80)?'X':'-', (comm_rx.resp[2] & 0x40)?'e':'-', (comm_rx.resp[2] & 0x20)?'I':'-', (comm_rx.resp[2] & 0x10)?'B':'-',
